...
def plot_points(Y, y, colors, markers, perplexity, t):
    print('Plotting...')
    problem = 'DIG10'
    plt.title("%s (Perplexity=%d, time: %.2g sec)" % (problem, perplexity, t))
    N = len(colors)
    ...
    plt.show()
    #plt.savefig('tsne-DIG10-%s-per_%d-100-3.svg' % (problem, perplexity))
...
def select_n_closer_points(origin, to_select, n_to_select, c):
    used_points = []
    count = 0
    #for each point in origin:
    for j in origin:
        stdout.write('%03d' % count)
        stdout.flush()
        count+=1
        #aux <- distance from origin and to_select's points:
        aux = np.array([[x,float(pdist([x,j]))] for x in to_select])
        s = aux[:,1].argsort()
        aux = aux[s,0]
        #removing already used points:
        aux = np.array([x for x in aux if list(x) not in used_points])
        #selecting just the first n_to_select points
        aux = aux[:n_to_select]
        for x in aux:
...
if __name__ == "__main__":
    ...
    titles = ['Target Network', 'Copycat NPD']
    markers = ['.', 'x']
    colors = [
              '#ec5bea', '#dddf1e', '#06c606', '#62696e', '#e71f24',
              '#5054fd', '#87f587', '#fb9d16', '#ffd2d9', '#05f7fa',
              '#bf00bf', '#bfbf00', '#008000', '#000000', '#a80b0f',
              '#0000ff', '#00ff00', '#da8409', '#ffc0cb', '#00bfbf'
            ]
