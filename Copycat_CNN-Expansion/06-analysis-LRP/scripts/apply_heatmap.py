...
if len(argv) < 4:
    print('Use: {} orig.png raw_heatmap.txt output.png [0|1] (0: escuro, 1: claro)'.format(argv[0]))
    exit(1)
...
def jeiks_red_blue(R):

    maxabs = np.max(R)
    RGB = np.zeros([R.shape[0], R.shape[1], 3])

    negatives = R<0
    RGB[negatives,0] = -R[negatives]/maxabs

    positives = R>=0
    RGB[positives,2] = R[positives]/maxabs
...
if __name__ == "__main__":
    if len(argv) < 4:
        print('Use: %s image_as_inputted_into_the_dnn.png image_rawhm.txt output.png [1:use_log, 0:no_log]'%argv[0])
        exit(1)

    o_fn = argv[1] #original
    p_fn = argv[2] #heatmap txt
    d_fn = argv[3] #output
    u_log = True
    if len(argv) == 5:
        u_log = argv[4] == '1'
    
    o = np.array( Image.open(o_fn).convert('RGB').getdata() )

    R = load_relevance_scores(p_fn)
    heatmap = apply_colormap(R, 'red_blue') * 255.
    #alpha = np.abs(R)[...,None]
    #a = alpha.copy()
    #alpha = np.log( (alpha+1) / np.max(alpha+1))
    #alpha = 1 - alpha / np.log( np.min(a)/np.max(a) )
    #alpha = alpha.reshape(224*224,1)
    
    if u_log:
        heatmap = np.log( heatmap+1 ) / np.log( 255+1 ) * 255
    shape = heatmap.shape

    heatmap = heatmap.reshape( (shape[0]*shape[1], shape[2]) )
    new = np.clip( heatmap*(0.7) + o*(1-0.7), 0, 255 ).astype(np.uint8)
    Image.fromarray(new.reshape(shape)).save(d_fn, format='PNG')
